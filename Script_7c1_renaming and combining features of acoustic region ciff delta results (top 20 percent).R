...
# Add site column based on the sign in the cliff_delta column
data1$site <- ifelse(data1$cliff_delta < 0, 'Olakara', 'Munipara')
...
# Extract the top 20 percent cliff_delta
top_20_percent <- data1_filtered %>% 
  top_n(n = nrow(data1_filtered) * 0.2, wt = absolute_cliff_delta) # change the percentage here

# Define the output file path
output_file_path <- "D:/Acoustic_Regions_filtered_Raven_Events_All_Region/output/Minute_Regularisation_models_cliff_delta/For Renaming and Combining Features of Acoustic Region Cliff Delta Results (top 20 percent).xlsx"
