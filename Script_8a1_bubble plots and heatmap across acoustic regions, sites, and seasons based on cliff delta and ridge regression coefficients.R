...
output_dir <- "D:/Acoustic_Regions_filtered_Raven_Events_All_Region/output/Minute_Regularisation_models_cliff_delta/"

# Read the data from the xlsx file
merged_data <- read_excel(paste0(output_dir, "Merged Acoustic Region Cliff Delta Results with Site-Wise Ridge Model Coefficients.xlsx"))
...
names(merged_data)[names(merged_data) == "absolute_cliff_delta"] <- "Cliff_Delta"
...
max_cliff_delta <- merged_data %>% group_by(`Acoustic Regions`, Sites, Seasons, Indices) %>% summarise(max_cliff_delta = ifelse(any(!is.na(Cliff_Delta)), max(Cliff_Delta, na.rm = TRUE), NA), absolute_cliff_delta = round(first(Cliff_Delta), 1), .groups = "drop")

# Join max_cliff_delta back to merged_data and sort by max_cliff_delta in ascending order
merged_data <- merged_data %>% left_join(max_cliff_delta, by = c("Acoustic Regions", "Sites", "Seasons", "Indices")) %>% arrange(desc(max_cliff_delta))

# Create a factor for Acoustic Regions with levels ordered by max_cliff_delta
merged_data$`Acoustic Regions` <- factor(merged_data$`Acoustic Regions`, levels = unique(merged_data$`Acoustic Regions`[order(merged_data$max_cliff_delta)]))

# Create a new column that combines Indices and absolute_cliff_delta
merged_data$combined <- paste0(merged_data$Indices, " (", merged_data$absolute_cliff_delta, ")")
...
ggpairs(merged_data[, c("Coefficients", "Cliff_Delta", "Acoustic Regions", "Indices", "Sites", "Seasons", "Events")])
ggsave(filename = "above_0_7__20%_top_Cliff_Delta_Pairwise scatterplot showing interactions of Coefficients, Cliff_Delta, Acoustic Regions, Indices, Site, and Season_Final.png", plot = last_plot(), path = output_dir, width = 10, height = 10, dpi = 300)

########################
# Create the scatterplot matrix
plot <- ggpairs(merged_data[, c("Coefficients", "Cliff_Delta", "Acoustic Regions", "Indices", "Sites", "Seasons", "Events")],
                lower = list(continuous = wrap("points", alpha = 0.5)),
                upper = list(continuous = wrap("cor", size = 3)),
                diag = list(continuous = wrap("barDiag")),
                progress = FALSE) +
  theme_bw() +
  theme(text = element_text(size = 14), # Increase size to 14
        axis.text.x = element_text(angle = 90, vjust = 0.5, hjust=1, size = 12), # Rotate x-axis labels and increase size to 15
        axis.text.y = element_text(angle = 0, vjust = 0.5, hjust=1, size = 12), # Rotate y-axis labels and increase size to 15
        strip.text = element_text(size = 12), # Increase facet label size to 14
        plot.margin = margin(1, 1, 1, 3, "cm")) # Increase left margin

# Save the plot
ggsave(filename = "above_0_7__20%_top_Cliff_Delta_Pairwise scatterplot showing interactions of Coefficients, Cliff_Delta, Acoustic Regions, Indices, Site, and Season1.png", plot = plot, path = output_dir, width = 20, height = 12, dpi = 300)
########################
...
if(nrow(merged_data) > 0) {
  ...
  ggsave(filename = paste0(output_dir, "/above_0_7__20%_top_Cliff_Delta_Heatmap_of_Coefficients_by_Acoustic Region_Index_and_Events__FINAL2.png"), plot = heatmap_plot_coefficient, width = 10, height = 8)
} else {
...
# Save the plot
ggsave(filename = "above_0_7__20%_top_Cliff_Delta_Heatmap_of_Coefficients_by_Region_Index_and_Events__FINAL3.png", plot = last_plot(), path = output_dir, width = 10, height = 10, dpi = 300)
...
if(nrow(merged_data) > 0) {
  bubble_plot <- ggplot(merged_data, aes(x = Events, y = combined, size = Coefficients, color = Events)) +
    ...
    theme(axis.text.x = element_text(angle = 90, hjust = 1, vjust = 0.5),  # Adjust this line
          axis.text.y = element_text(size = rel(1.05))) +  # Adjust the font size of the y-axis labels
    labs(x = "Events", y = "Acoustic Regions\nIndices (Cliff_Delta)", size = "Coefficients", color = "Events",
  ...
  ggsave(filename = paste0(output_dir, "/above_0_7__20%_top_Cliff_Delta_Bubble_Plot_of_Coefficients_by_Region_Index_and_Species__FINAL1.png"), plot = bubble_plot, width = 10, height = 8)
}
...
ggsave(filename = "above_0_7__20%_top_Cliff_Delta_Bubble_Plot_of_Coefficients_by_Region_Index_and_Events__FINAL2.png", plot = last_plot(), path = output_dir, width = 10, height = 10, dpi = 300)
...
write.xlsx(merged_data[, c("Coefficients", "Cliff_Delta", "Acoustic Regions", "Indices", "Sites", "Seasons", "Events")], 
           file.path(output_dir, "above_0_7__20%_top_Cliff_Delta_Bubble_pairwise_scatterplot_data.xlsx"))

# Save the data used for the heatmap to an Excel file
write.xlsx(merged_data, file.path(output_dir, "above_0_7__20%_top_Cliff_Delta_Bubble_heatmap_data.xlsx"))

# Save the data used for the bubble plot to an Excel file
write.xlsx(merged_data, file.path(output_dir, "above_0_7__20%_top_Cliff_Delta_Bubble_bubble_plot_data.xlsx"))
...
if(nrow(merged_data) > 0) {
  bubble_plot <- ggplot(merged_data, aes(x = Events, y = combined, size = Coefficients, color = Events)) +
    geom_point(alpha = 0.5) +
    facet_grid(. ~ Seasons) +
    scale_size_continuous(range = c(1,30)) +  # Increase the maximum size of the bubbles
    theme(axis.text.x = element_text(angle = 90, hjust = 1, vjust = 0.5, size = rel(2.8)),  # Increase the font size of the x-axis labels
          axis.text.y = element_text(size = rel(2.6)),  # Adjust the font size of the y-axis labels
          axis.title.x = element_text(size = rel(2.5)),  # Increase the font size of the x-axis title
          axis.title.y = element_text(size = rel(2.5)),  # Increase the font size of the y-axis title
          legend.text = element_text(size = rel(2.05)),  # Increase the font size of the legend
          legend.title = element_text(size = rel(2.5)),  # Increase the font size of the legend title
          plot.title = element_text(size = rel(2.5)),  # Increase the font size of the general title
          plot.subtitle = element_text(size = rel(2.5)),  # Increase the font size of the subtitle
          strip.text = element_text(size = rel(2.05))) +  # Increase the font size of the facet labels
    labs(x = "Events", y = "Acoustic Regions\nIndices (Cliff_Delta)", size = "Coefficients", color = "Events",
         title = "Bubble Plot of Coefficients by Acoustic Regions, Indices and Events",
         subtitle = "Each cell represents the coefficient of an event for a given region:index combination")
  
  # Save the bubble plot as a PNG file with an appropriate size
  ggsave(filename = paste0(output_dir, "/above_0_7_without_Sites__20%_top_Cliff_Delta_Bubble_Plot_of_Coefficients_by_Region_Index_and_Species__FINAL1.png"), plot = bubble_plot, width = 25, height = 25)
}
...
if(nrow(merged_data) > 0) {
  bubble_plot <- ggplot(merged_data, aes(x = Events, y = combined, size = Coefficients, color = Events)) +
    ...
    scale_size_continuous(range = c(1,10)) +
    theme(axis.text.x = element_text(angle = 90, hjust = 1, vjust = 0.5),  # Adjust this line
          axis.text.y = element_text(size = rel(1.2))) +  # Adjust the font size of the y-axis labels
    labs(x = "Events", y = "Acoustic Regions\nIndices (Cliff_Delta)", size = "Coefficients", color = "Events",
         title = "Bubble Plot of Coefficients by Acoustic Regions, Indices and Events",
         subtitle = "Each cell represents the coefficient of an event for a given region:index combination")
  
  # Save the bubble plot as a PNG file with an appropriate size
  ggsave(filename = paste0(output_dir, "/above_0_7_with_Sites_20%_top_Cliff_Delta_Bubble_Plot_of_Coefficients_by_Region_Index_and_Species__FINAL1.png"), plot = bubble_plot, width = 15, height = 13)
}
...
if(nrow(merged_data) > 0) {
  ...
  # Save the bubble plot as a PNG file with an appropriate size
  ggsave(filename = paste0(output_dir, "/above_0_7_with_Sites2_20%_top_Cliff_Delta_Bubble_Plot_of_Coefficients_by_Region_Index_and_Species__FINAL1.png"), plot = bubble_plot, width = 15, height = 13)
}