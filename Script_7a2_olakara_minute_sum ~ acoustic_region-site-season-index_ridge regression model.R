...
# Set file paths
input_file <- "D:/Acoustic Region Analysis 2023/Results/minute_frequency_time_summaries_acoustic_regions.xlsx"
output_dir <- "D:/Acoustic_Regions_filtered_Raven_Events_All_Region/output/Minute_Regularisation_models_minute_sum/OLAKARA_minute_sum ~ acoustic_region-site-season-index/feature_engineered"
...
# Filter data for Olakara site only
data <- data %>%
  filter(site == "Olakara")

# Data preprocessing
data <- data %>%
  mutate(across(c(index, acoustic_region, season), as.factor)) %>%
  mutate(minute_sum = as.integer(minute_sum)) %>%
  filter(!is.na(minute_sum))

# Create a model matrix for the predictors
x <- model.matrix(minute_sum ~ acoustic_region:season:index, data = data)[,-1]  # remove intercept column
...
threshold <- 0.01 # replace with the desired threshold (Since threshold #1 didn't yield any results,it was reduced to 0.01 to explore prominent (though weak) relationships
...
coefs_df_ridge$Variable <- gsub("acoustic_region|index|site|season", "", coefs_df_ridge$Variable)
coefs_df_lasso$Variable <- gsub("acoustic_region|index|site|season", "", coefs_df_lasso$Variable)
coefs_df_elastic_net$Variable <- gsub("acoustic_region|index|site|season", "", coefs_df_elastic_net$Variable)
...
# Assuming coefs_df_ridge is already in the environment
...
# Remove terms from variable names
coefs$Feature <- gsub("acoustic_region|index|site|season", "", coefs$Feature)
...
plot_coefs <- function(coefs, title) {
  ...
  ggsave(filename = paste0(output_dir, paste0(gsub(" ", "_", title), "_OLAKARA_Plot_best_Ridge_model.pdf")), plot = p1, width = 10, height = 10, units = "in")
}
...
png(file = paste(output_dir, "/OLAKARA_Histogram_Coefficients_best_Ridge_model.png", sep = ""))
hist(coefs_df_ridge$Coef, main="Histogram of Coefficients", xlab="Coefficient")
dev.off()

# Save the boxplot to a PNG file
png(file = paste(output_dir, "/OLAKARA_Boxplot_Coefficients_best_Ridge_model.png", sep = ""))
...
# Assuming 'coefs_df_ridge' is the Ridge model coefficients
...
# Remove terms from variable names
df$Feature <- gsub("acoustic_region:|index:|season:|site:|Species.names:|", "", df$Feature)
# Save the sorted coefficients to a CSV file
write.csv(df, file = paste(output_dir, "/OLAKARA_coefficients_descending_best_Ridge_model.csv", sep = ""))
